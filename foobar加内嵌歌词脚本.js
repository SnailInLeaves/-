...
function start_search(info, callback) {
    ...
    if (xmlHttp.Status == 200) {
        ...
        if (xmlHttp.Status == 200) {
			...
			if(!lrc_order.length) lrc_order = ["newtype","merge","origin","tran"];
                ...
                switch (lrc_order[key]) {
                    case "merge" :
                        if (issetlrc && issettran) {
                            ...
                            newLyric.Source = "(千A)" + get_my_name();
                    ...
                    case "origin" :
                        if (issetlrc) {
                            ...
                            newLyric.Title = res_name + outstr1;
                            newLyric.Artist = res_artist + outstr2;
                            newLyric.Source = "(蝸O)" + get_my_name();
                            callback.AddLyric(newLyric);
                        }
                        break;
                    case "tran" :
                        if (issettran) {
                            ...
                            newLyric.Title = res_name + outstr1;
                            newLyric.Artist = res_artist + outstr2;
                            newLyric.Source = "(牛T)" + get_my_name();
                            callback.AddLyric(newLyric);
                        }
                        break;
                    case "newtype":
                        if (issetlrc && issettran) {
                            newLyric.LyricText = lrc_newtype(ncm_lrc.lrc.lyric, tranlrc);
                            newLyric.Title = res_name + outstr1;
                            newLyric.Artist = res_artist + outstr2;
                            newLyric.Source = "(葉N)" + get_my_name();
                            callback.AddLyric(newLyric);
                        }
                        break;
                }
...
function lrc_newtype(olrc, tlrc) {
    ...
    for(var kk=0;kk<l_r;kk++){
        ...
        t=r[kk][1];
        lrc.push(olrc[o]);
        var t_lrc=t&&tlrc[t].substr(set + 1)?bracket[0]+tlrc[t].substr(set + 1)+bracket[1]:" ";
        if(kk+2>l_r) break;
        if(r[kk+1][2]){
			...
			if(savefix) {
				...
				var time="["+Math.floor(ut/60)+":"+(ut%60).toFixed(set-7)+"]";
				debug&&console(time);
			}else var time="["+timeb+"]";
        }else{
			...
			var ut=x*60+y+4;
			var time= "["+Math.floor(ut/60)+":"+Math.floor(ut%60)+z+"]";
			debug&&console(time);
        }
